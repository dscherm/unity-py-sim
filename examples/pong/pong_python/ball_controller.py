...
class BallController(MonoBehaviour):
    ...
    def __init__(self):
        super().__init__()
        self.initial_speed: float = 6.0
        self.speed_increase: float = 0.3
        self.current_speed: float = 6.0
    ...
    def reset(self):
        ...
        self.rb.move_position(Vector2(0, 0))
    ...
    def on_collision_enter_2d(self, collision):
        if collision.game_object.tag == "Paddle":
            ...
            paddle_x = collision.game_object.transform.position.x
            hit_y = self.transform.position.y - collision.game_object.transform.position.y
            normalized_hit = max(-1.0, min(1.0, hit_y / 1.0))
            # Ball goes away from the paddle it hit
            x_dir = 1.0 if paddle_x < 0 else -1.0
