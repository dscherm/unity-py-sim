...
_DEFAULT_PACKAGES: dict[str, str] = {
    ...
    # M-7 phase 2 — PlayMode validation via Unity Test Framework.
    # Resolved transitively today (CoPlay pulls 1.6.0); declare explicitly
    # so the [UnityTest] runner is reliably available for home_machine.yml.
...
def scaffold_project(
    ...
    *,
    ...
    # 13. Write PlayMode test asmdef + .cs (M-7 phase 2).  Unity Test Framework
    # picks the test up via `Unity -runTests -testPlatform PlayMode` so the
    # home_machine workflow can fail-red on Awake/Start/Update NullRefs that
    # the deploy-only batchmode harness would silently ignore.
    _write_playmode_test(output_dir, game_name)

    # 14. Heal any existing Prefabs/*.prefab whose m_Script GUIDs drifted
...
def _write_playmode_test(output_dir: Path, game_name: str) -> None:
    """Write Assets/Tests/PlayMode/PlayModeTests.{cs,asmdef} (M-7 phase 2).

    Renders ``tools/home_machine_playmode_test.cs.j2`` (substituting the
    game name into the file header for traceability) and emits the matching
    asmdef so Unity Test Framework picks the test up via
    ``Unity -runTests -testPlatform PlayMode``.
    """
    from src.exporter.prefab_generator import _deterministic_guid

    tests_dir = output_dir / "Assets" / "Tests" / "PlayMode"
    tests_dir.mkdir(parents=True, exist_ok=True)

    template_path = (
        Path(__file__).resolve().parent.parent.parent
        / "tools"
        / "home_machine_playmode_test.cs.j2"
    )
    rendered = template_path.read_text(encoding="utf-8").replace(
        "{{ game_name }}", game_name
    )

    # PlayModeTests.cs + .meta
    cs_path = tests_dir / "PlayModeTests.cs"
    cs_path.write_text(rendered, encoding="utf-8")
    ...
    asmdef_path = tests_dir / "PlayModeTests.asmdef"
    asmdef_path.write_text(
        json.dumps(asmdef, indent=4) + "\n", encoding="utf-8"
    )

    asmdef_guid = _deterministic_guid("asmdef:PlayModeTests")
    (tests_dir / "PlayModeTests.asmdef.meta").write_text(
        "fileFormatVersion: 2\n"
        f"guid: {asmdef_guid}\n"
