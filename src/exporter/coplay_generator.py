...
def generate_scene_script(
    ...
    lines.append("    {")
    lines.append("        // FU-4 editor-guard: EditorSceneManager.NewScene and")
    lines.append("        // SaveOpenScenes throw InvalidOperationException when the")
    lines.append("        // editor is in Play mode.  Fail fast with a readable message")
    lines.append("        // rather than corrupting state mid-play.")
    lines.append("        if (EditorApplication.isPlayingOrWillChangePlaymode || EditorApplication.isPlaying)")
    lines.append("        {")
    lines.append("            return \"[skipped] scene setup refused: editor is in Play mode\";")
    lines.append("        }")
    lines.append("")
    ...
    lines.append("")
    lines.append("        // === SAVE ===")
    lines.append("        // FU-4 scene-save-path: force the canonical scene path so")
    lines.append("        // CoPlay's save_scene MCP call doesn't silently drop Scene.unity")
    lines.append("        // at Assets/ root (see coplay_generator_gaps.md P2 observation).")
    lines.append("        const string _scenePath = \"Assets/_Project/Scenes/Scene.unity\";")
    lines.append("        var _activeScene = UnityEditor.SceneManagement.EditorSceneManager.GetActiveScene();")
    lines.append("        UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(_activeScene);")
    lines.append("        if (string.IsNullOrEmpty(_activeScene.path))")
    lines.append("        {")
    lines.append("            System.IO.Directory.CreateDirectory(\"Assets/_Project/Scenes\");")
    lines.append("            UnityEditor.SceneManagement.EditorSceneManager.SaveScene(_activeScene, _scenePath);")
    lines.append("        }")
    lines.append("        else")
    lines.append("        {")
...
def generate_validation_script(
    ...
    lines.append("        // FU-4 FindObjectsByType migration — FindObjectsOfType is deprecated")
    lines.append("        // in Unity 6 (CS0618); FindObjectsByType requires an explicit sort mode.")
    lines.append("        var allGOs = Object.FindObjectsByType<GameObject>(FindObjectsSortMode.None);")
