...
def _check_parity_test(row: ParityRow, test_dir: Path) -> bool:
    ...
    Files containing the `# PARITY_SCAFFOLD_SKELETON` marker are excluded — those
    are unfilled auto-generated stubs and don't count.

    Files containing the `# PARITY_SCAFFOLD_PARKED` marker DO count for the class
    they cover: parked APIs have been explicitly decided as out-of-scope for
    headless dual-path testing (Audio, UI, SceneManager, DOTween) and the parked
    notice is the documented decision.
    """
    ...
    for test_file in test_dir.rglob("test_*.py"):
        ...
        if "PARITY_SCAFFOLD_SKELETON" in text:
            continue
        text_lower = text.lower()
        if needle_class not in text_lower:
            continue
        if not needle_member:
            return True
        if needle_member in text_lower:
            return True
    return False
